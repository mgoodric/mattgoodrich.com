...
The guide has two layers: the **Writing Rules** are the general baseline that applies to anything written for or to Matt. The **Voice guide** below the rules describes Matt's specific patterns from his published posts. Where the two collide, the **Voice exceptions** section names the specific carve-outs.
...
The voice-derived guide below describes signature patterns from Matt's published posts. Where it differs from the Writing Rules above, these scoped exceptions apply for **posts written in Matt's voice for mattgoodrich.com**. The Writing Rules still govern chat, drafts, and anything not in a published-post voice.

## Em-dashes (Section 3)

Em-dashes are allowed sparingly. A handful per post, zero is fine, never a habitual substitute for commas. See "Em-dashes" under "Paragraph and sentence rhythm" below for the test. The Section 3 "No em dashes" rule is too strict for Matt's published prose — periods, commas, and parentheses stay the first choice, but a well-earned em-dash is part of the voice.

## Named dichotomies and structural framings (Section 5)

The reframe ban does not apply to:

- **Named dichotomies** introduced as concepts the post will use throughout: *discovered debt* vs *decided debt*, *complicated* vs *complex*, *find* vs *fix*, *direct* vs *transitive dependencies*
- **Structural framings** near the closing that distill the thesis: *the floor, not the ceiling*, *find vs. fix as a gap*
- **Named multi-bucket sorts** when the territory genuinely partitions: *already mitigated / genuinely new / old problem, new actor*

These are signature moves. The ban targets cheap rhetorical reframes ("This isn't X. This is Y.") used for fake depth — not concept-naming or thesis-distilling work. See "Dichotomies and named concepts" below for how to introduce one.

## Structural metaphors (Section 6)

Short structural metaphors that name an argument's spine are allowed even when they don't pass the 5-test permission gate. *The floor, not the ceiling* is doing concept-naming work, not decoration. The Section 6 bans still apply to ordinary metaphors used for color — gardening, chess, journey, ecosystem, and the banned verb list all still hold.

The test: is the metaphor naming a concept the post returns to, or is it dressing up a clear point? If the former, keep it. If the latter, cut it.

---

# Voice guide

The sections below describe Matt's voice as derived from his published posts. Read them after the Writing Rules — they add Matt-specific patterns on top of the general baseline.
